...
import { addPath, getBooleanInput, getIDToken, getInput, info, warning, setFailed, platform, summary, } from '@actions/core';
...
const handleCache = async (config, key) => {
    try {
        const cachePath = join(config.cacheDirectory, `${GITHUB_API.repo}${config.extended ? '_extended' : ''}`);
...
const saveToCache = async (config, semver, key) => {
    const cachePath = join(config.cacheDirectory, `${GITHUB_API.repo}${config.extended ? '_extended' : ''}`, semver, config.osArch);
    try {
...
const verifyChecksum = async (downloadPath, release, assetName, config) => {
    ...
    summary.addRaw(`Fetching checksum file from: [checksums.txt](${checksumAsset.browser_download_url})\n`);
...
const installHugo = async (semver, downloadUrl, assetName, config, release) => {
    ...
    const cachedPath = await cacheDir(extractedFolder, `${GITHUB_API.repo}${config.extended ? '_extended' : ''}`, semver, config.osArch);
    addPath(cachedPath);
...
export const main = async () => {
    try {
        ...
        console.log('ALIBABA_CLOUD_OIDC_TOKEN_FILE:', env.ALIBABA_CLOUD_OIDC_TOKEN_FILE);
        ...
        if (!cachedPath) {
            ...
            if (!asset && config.osPlatform) {
                ...
                if (asset) {
                    info(`Asset not found with platform '${config.osPlatform}', retrying with capitalized platform '${capitalizedPlatform}'`);
                    summary.addRaw(`Asset not found with platform '**${config.osPlatform}**', retrying with '**${capitalizedPlatform}**'\n`);
                }
            }
            if (!asset) {
        ...
        summary.addRaw(`Executing command: **${config.executable} ${argsArray.join(' ')}**\n`);
        ...
        summary.write();
        setFailed(`Action failed: ${error instanceof Error ? error.message : 'Unknown error'}`);
    }
};
main();
